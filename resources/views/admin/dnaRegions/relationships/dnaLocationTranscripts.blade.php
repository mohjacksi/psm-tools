    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table
                    class=" table table-bordered table-stripd table-hover datatable datatable-dnaLocationTranscripts">
                    <thead>
                        <tr>
                            <th width="10">
                                ...
                                <td>
                                    @can('transcript_show')
                                        <a class="btn btn-xs btn-primary"
                                            href="{{ route('admin.transcripts.show', $transcript->id) }}">
                                            {{ trans('global.view') }}
                                        </a>
                                    @endcan
                                    ...
                                    @can('transcript_edit')
                                        <a class="btn btn-xs btn-info"
                                            href="{{ route('admin.transcripts.edit', $transcript->id) }}">
                                            {{ trans('global.edit') }}
                                        </a>
                                    @endcan

                                    @can('transcript_delete')
                                        <form action="{{ route('admin.transcripts.destroy', $transcript->id) }}"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-dnaLocationTranscripts:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
