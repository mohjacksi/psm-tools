...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-peptidePeptidePsms">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <tr data-entry-id="{{ $peptidePsm->id }}">
                            ...
                            <td>
                                @can('peptide_psm_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.peptide-psms.show', $peptidePsm->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('peptide_psm_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.peptide-psms.edit', $peptidePsm->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('peptide_psm_delete')
                                    <form action="{{ route('admin.peptide-psms.destroy', $peptidePsm->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-peptidePeptidePsms:not(.ajaxTable)').DataTable({ buttons: dtButtons })
