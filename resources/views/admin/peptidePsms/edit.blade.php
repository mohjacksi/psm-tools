...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.peptidePsm.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.peptide-psms.update", [$peptidePsm->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="name">{{ trans('cruds.peptidePsm.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $peptidePsm->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('peptide') ? 'is-invalid' : '' }}" name="peptide_id" id="peptide_id">
                    @foreach($peptides as $id => $entry)
                        <option value="{{ $id }}" {{ (old('peptide_id') ? old('peptide_id') : $peptidePsm->peptide->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('psm') ? 'is-invalid' : '' }}" name="psm_id" id="psm_id">
                    @foreach($psms as $id => $entry)
                        <option value="{{ $id }}" {{ (old('psm_id') ? old('psm_id') : $peptidePsm->psm->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('psm'))
                    <div class="invalid-feedback">
