...
@can('peptide_psm_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            ...
            @include('csvImport.modal', ['model' => 'PeptidePsm', 'route' => 'admin.peptide-psms.parseCsvImport'])
        </div>
    </div>
@endcan
<div class="card">
    ...
    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-PeptidePsm">
            <thead>
                <tr>
                    <th width="10">
                ...
                <tr>
                    ...
                    <td>
                        <select class="search">
                            <option value>{{ trans('global.all') }}</option>
                            @foreach($psms as $key => $item)
                                <option value="{{ $item->spectra }}">{{ $item->spectra }}</option>
                            @endforeach
                        </select>
                    </td>
                    <td>
...
@endcan
  ...
  let dtOverrideGlobals = {
    ...
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.peptide-psms.index') }}",
    columns: [
      { data: 'placeholder', name: 'placeholder' },
{ data: 'id', name: 'id' },
{ data: 'name', name: 'name' },
{ data: 'peptide_sequence', name: 'peptide.sequence' },
{ data: 'psm_spectra', name: 'psm.spectra' },
{ data: 'actions', name: '{{ trans('global.actions') }}' }
  ...
  let table = $('.datatable-PeptidePsm').DataTable(dtOverrideGlobals);
