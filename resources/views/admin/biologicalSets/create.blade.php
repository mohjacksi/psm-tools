...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.biological-sets.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('labeling_number') ? 'is-invalid' : '' }}" type="number" name="labeling_number" id="labeling_number" value="{{ old('labeling_number', '') }}" step="1">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('experiments') ? 'is-invalid' : '' }}" name="experiments[]" id="experiments" multiple required>
                    @foreach($experiments as $id => $experiment)
                        <option value="{{ $id }}" {{ in_array($id, old('experiments', [])) ? 'selected' : '' }}>{{ $experiment }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('stripe') ? 'is-invalid' : '' }}" name="stripe_id" id="stripe_id">
                    @foreach($stripes as $id => $entry)
                        <option value="{{ $id }}" {{ old('stripe_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('fragment_method') ? 'is-invalid' : '' }}" name="fragment_method_id" id="fragment_method_id">
                    @foreach($fragment_methods as $id => $entry)
                        <option value="{{ $id }}" {{ old('fragment_method_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
