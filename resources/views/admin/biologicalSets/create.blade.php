            ...
            <form method="POST" action="{{ route('admin.biological-sets.store') }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('labeling_number') ? 'is-invalid' : '' }}" type="number"
                        name="labeling_number" id="labeling_number" value="{{ old('labeling_number', '') }}"
                        step="1">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('experiments') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($experiments as $id => $experiment)
                            <option value="{{ $id }}"
                                {{ in_array($id, old('experiments', [])) ? 'selected' : '' }}>{{ $experiment }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('strip') ? 'is-invalid' : '' }}" name="strip_id"
                        ...
                        @foreach ($strips as $id => $entry)
                            <option value="{{ $id }}" {{ old('strip_id') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('strip'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('fragment_method') ? 'is-invalid' : '' }}"
                        ...
                        @foreach ($fragment_methods as $id => $entry)
                            <option value="{{ $id }}"
                                {{ old('fragment_method_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
