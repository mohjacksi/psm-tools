...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-stripd table-hover datatable datatable-categoryPeptides">
                <thead>
                    <tr>
                        <th width="10">
                            ...
                            <td>
                                @can('peptide_show')
                                    <a class="btn btn-xs btn-primary"
                                        href="{{ route('admin.peptides.show', $peptide->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan
                                ...
                                @can('peptide_edit')
                                    <a class="btn btn-xs btn-info"
                                        href="{{ route('admin.peptides.edit', $peptide->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('peptide_delete')
                                    <form action="{{ route('admin.peptides.destroy', $peptide->id) }}" method="POST"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-categoryPeptides:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
