    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.people.update", [$person->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $person->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('projects') ? 'is-invalid' : '' }}" name="projects_id" id="projects_id">
                    @foreach($projects as $id => $entry)
                        <option value="{{ $id }}" {{ (old('projects_id') ? old('projects_id') : $person->projects->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
