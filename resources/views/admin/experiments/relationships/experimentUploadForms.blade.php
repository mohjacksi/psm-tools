...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-experimentUploadForms">
                <thead>
                    <tr>
                        <th width="10">
                        ...
                        <tr data-entry-id="{{ $uploadForm->id }}">
                            ...
                            <td>
                                @can('upload_form_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.upload-forms.show', $uploadForm->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('upload_form_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.upload-forms.edit', $uploadForm->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('upload_form_delete')
                                    <form action="{{ route('admin.upload-forms.destroy', $uploadForm->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-experimentUploadForms:not(.ajaxTable)').DataTable({ buttons: dtButtons })
