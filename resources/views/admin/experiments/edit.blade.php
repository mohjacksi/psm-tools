...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.experiment.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.experiments.update", [$experiment->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="name">{{ trans('cruds.experiment.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $experiment->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('project') ? 'is-invalid' : '' }}" name="project_id" id="project_id">
                    @foreach($projects as $id => $entry)
                        <option value="{{ $id }}" {{ (old('project_id') ? old('project_id') : $experiment->project->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $experiment->date) }}">
                @if($errors->has('date'))
                    <div class="invalid-feedback">
                        {{ $errors->first('date') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('method') ? 'is-invalid' : '' }}" name="method" id="method">
                    ...
                    @foreach(App\Models\Experiment::METHOD_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('method', $experiment->method) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('allowed_missed_cleavage') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="allowed_missed_cleavage" id="allowed_missed_cleavage" value="1" {{ $experiment->allowed_missed_cleavage || old('allowed_missed_cleavage', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('expression_threshold') ? 'is-invalid' : '' }}" type="number" name="expression_threshold" id="expression_threshold" value="{{ old('expression_threshold', $experiment->expression_threshold) }}" step="0.0001">
            ...
            <div class="form-group">
                <label for="species">{{ trans('cruds.experiment.fields.species') }}</label>
                <input class="form-control {{ $errors->has('species') ? 'is-invalid' : '' }}" type="text" name="species" id="species" value="{{ old('species', $experiment->species) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('reference_protein_source') ? 'is-invalid' : '' }}" type="text" name="reference_protein_source" id="reference_protein_source" value="{{ old('reference_protein_source', $experiment->reference_protein_source) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('other_protein_source') ? 'is-invalid' : '' }}" type="text" name="other_protein_source" id="other_protein_source" value="{{ old('other_protein_source', $experiment->other_protein_source) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('psm_file_name') ? 'is-invalid' : '' }}" type="text" name="psm_file_name" id="psm_file_name" value="{{ old('psm_file_name', $experiment->psm_file_name) }}">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('metadata') ? 'is-invalid' : '' }}" name="metadata" id="metadata">{!! old('metadata', $experiment->metadata) !!}</textarea>
