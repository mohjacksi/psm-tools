...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.peptideProtein.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.peptide-proteins.update", [$peptideProtein->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="name">{{ trans('cruds.peptideProtein.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $peptideProtein->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('peptide') ? 'is-invalid' : '' }}" name="peptide_id" id="peptide_id">
                    @foreach($peptides as $id => $entry)
                        <option value="{{ $id }}" {{ (old('peptide_id') ? old('peptide_id') : $peptideProtein->peptide->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('protein') ? 'is-invalid' : '' }}" name="protein_id" id="protein_id">
                    @foreach($proteins as $id => $entry)
                        <option value="{{ $id }}" {{ (old('protein_id') ? old('protein_id') : $peptideProtein->protein->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
