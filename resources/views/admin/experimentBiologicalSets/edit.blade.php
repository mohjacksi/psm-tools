...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.experimentBiologicalSet.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.experiment-biological-sets.update", [$experimentBiologicalSet->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="name">{{ trans('cruds.experimentBiologicalSet.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $experimentBiologicalSet->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('set') ? 'is-invalid' : '' }}" type="text" name="set" id="set" value="{{ old('set', $experimentBiologicalSet->set) }}">
                @if($errors->has('set'))
                    <div class="invalid-feedback">
                        {{ $errors->first('set') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('experiment') ? 'is-invalid' : '' }}" name="experiment_id" id="experiment_id">
                    @foreach($experiments as $id => $entry)
                        <option value="{{ $id }}" {{ (old('experiment_id') ? old('experiment_id') : $experimentBiologicalSet->experiment->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
