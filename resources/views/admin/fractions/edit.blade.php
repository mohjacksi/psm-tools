...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.fraction.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.fractions.update", [$fraction->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="name">{{ trans('cruds.fraction.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $fraction->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('biological_set') ? 'is-invalid' : '' }}" name="biological_set_id" id="biological_set_id">
                    @foreach($biological_sets as $id => $entry)
                        <option value="{{ $id }}" {{ (old('biological_set_id') ? old('biological_set_id') : $fraction->biological_set->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('spectra_file_name') ? 'is-invalid' : '' }}" type="text" name="spectra_file_name" id="spectra_file_name" value="{{ old('spectra_file_name', $fraction->spectra_file_name) }}">
