...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class=" table table-bordered table-striped table-hover datatable datatable-projectExperiments">
                <thead>
                    <tr>
                        <th width="10">
                ...
                <tbody>
                    @foreach($experiments as $key => $experiment)
                        <tr data-entry-id="{{ $experiment->id }}">
                            ...
                            <td>
                                @can('experiment_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.experiments.show', $experiment->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('experiment_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.experiments.edit', $experiment->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('experiment_delete')
                                    <form action="{{ route('admin.experiments.destroy', $experiment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-projectExperiments:not(.ajaxTable)').DataTable({ buttons: dtButtons })
