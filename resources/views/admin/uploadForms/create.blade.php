            ...
            <form method="POST" action="{{ route("admin.projects.store") }}" enctype="multipart/form-data" id="projectForm">
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('public') ? 'is-invalid' : '' }}">
                        ...
                        <input class="form-check-input" type="checkbox" name="public" id="public" value="1" {{ old('public', 0) == 1 ? 'checked' : '' }}>
                        <label class="form-check-label" for="public">{{ trans('cruds.project.fields.public') }}</label>
                    </div>
  ...
  <div class="modal fade" id="experimentModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
            <form method="POST" action="{{ route("admin.experiments.store") }}" enctype="multipart/form-data" id="experimentForm">
                @csrf
                <div class="form-group">
  ...
  <div class="modal fade" id="sampleModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <div class="modal-body">
            <form method="POST" action="{{ route("admin.samples.store") }}" enctype="multipart/form-data" id="sampleForm">
                @csrf
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('replicate_number') ? 'is-invalid' : '' }}" type="number" name="replicate_number" id="replicate_number" value="{{ old('replicate_number', '') }}" step="1">
...
$("#experimentForm").submit(function(e) {
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('.modal-backdrop').attr('class', '');
            $('#experiment_id').append('<option value="'+data.id+'">'+data.name+'</option>')
        }
    });
...
$("#sampleForm").submit(function(e) {
    ...
    $.ajax({
        ...
        success: function(data)
        {
            ...
            $('.modal-backdrop').attr('class', '');
            $('#sample_id').append('<option value="'+data.id+'">'+data.name+'</option>');
            sampleInputs();
        }
    });
