    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.biological-sets.update", [$biologicalSet->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            <label for="name">{{ trans('cruds.biologicalSet.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $biologicalSet->name) }}">
                            @if($errors->has('name'))
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="labeling_number" id="labeling_number" value="{{ old('labeling_number', $biologicalSet->labeling_number) }}" step="1">
                        ...
                        <div class="form-group">
                            <label class="required" for="experiments">{{ trans('cruds.biologicalSet.fields.experiment') }}</label>
                            <div style="padding-bottom: 4px">
                                <span class="btn btn-info btn-xs select-all" style="border-radius: 0">{{ trans('global.select_all') }}</span>
                                <span class="btn btn-info btn-xs deselect-all" style="border-radius: 0">{{ trans('global.deselect_all') }}</span>
                            </div>
                            <select class="form-control select2" name="experiments[]" id="experiments" multiple required>
                                @foreach($experiments as $id => $experiment)
                                    <option value="{{ $id }}" {{ (in_array($id, old('experiments', [])) || $biologicalSet->experiments->contains($id)) ? 'selected' : '' }}>{{ $experiment }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="stripe_id" id="stripe_id">
