                    ...
                    <div style="margin-bottom: 10px;" class="row">
                        <div class="col-lg-12">
                            ...
                            @include('csvImport.modal', [
                                'model' => 'Peptide',
                                'route' => 'admin.peptides.parseCsvImport',
                            ])
                        </div>
                    </div>
                @endcan
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class=" table table-bordered table-stripd table-hover datatable datatable-Peptide">
                                <thead>
                                    <tr>
                                        <th>
                                            ...
                                            <td>
                                                @can('peptide_show')
                                                    <a class="btn btn-xs btn-primary"
                                                        href="{{ route('frontend.peptides.show', $peptide->id) }}">
                                                        {{ trans('global.view') }}
                                                    </a>
                                                @endcan
                                                ...
                                                @can('peptide_edit')
                                                    <a class="btn btn-xs btn-info"
                                                        href="{{ route('frontend.peptides.edit', $peptide->id) }}">
                                                        {{ trans('global.edit') }}
                                                    </a>
                                                @endcan

                                                @can('peptide_delete')
                                                    <form action="{{ route('frontend.peptides.destroy', $peptide->id) }}"
                                                        method="POST"
...
@section('scripts')
    ...
    <script>
        $(function() {
            ...
            let table = $('.datatable-Peptide:not(.ajaxTable)').DataTable({
                buttons: dtButtons
            })
