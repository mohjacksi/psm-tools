            ...
            @can('experiment_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        ...
                        @include('csvImport.modal', ['model' => 'Experiment', 'route' => 'admin.experiments.parseCsvImport'])
                    </div>
                </div>
            @endcan
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-Experiment">
                            <thead>
                                <tr>
                                    <th>
                                ...
                                @foreach($experiments as $key => $experiment)
                                    <tr data-entry-id="{{ $experiment->id }}">
                                        ...
                                        <td>
                                            ...
                                            <input type="checkbox" disabled="disabled" {{ $experiment->allowed_missed_cleavage ? 'checked' : '' }}>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            @can('experiment_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.experiments.show', $experiment->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('experiment_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.experiments.edit', $experiment->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('experiment_delete')
                                                <form action="{{ route('frontend.experiments.destroy', $experiment->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@endcan
  ...
  let table = $('.datatable-Experiment:not(.ajaxTable)').DataTable({ buttons: dtButtons })
